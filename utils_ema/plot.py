...
class plotter:
    ...
    def plot_point_light(cls, point_light, size=1, opacity=1, color="red", frame=None):
        cls.plot_points(
            point_light.position, size=size, opacity=opacity, color=color, frame=frame
        )
    ...
    def save(cls, path):
        fig = cls.init_figure(
            max_corner=[-cls.max_corner, cls.max_corner], sliders=True
        )
        fig.write_html(path)
    ...
    @classmethod
    def show(cls, name="3D plot"):
        assert cls.frames is not None
        fig = cls.init_figure(
            title=name, max_corner=[-cls.max_corner, cls.max_corner], sliders=True
        )
        fig.show()
        # get data
        # frames = []
    ...
    def plot_sphere(cls, sphere, opacity=0.7, colorscale="Viridis", frame=None):
        ...
        for i in range(l.shape[0]):
            ...
            cls.plot_surface(
                np.concatenate(
                    (x.unsqueeze(-1), y.unsqueeze(-1), z.unsqueeze(-1)), axis=-1
                ),
                frame,
            )
    ...
    @classmethod
    def plot_aabb(cls, corners, opacity=0.4, color="lightcyan", frame=None):
    ...
    def plot_cam(cls, camera, size=0.1, frame=None):
        ...
        c01 = (
            o + camera.pix2dir(torch.LongTensor([0, camera.intr.resolution[1]])) * size
        )
        c10 = (
            o + camera.pix2dir(torch.LongTensor([camera.intr.resolution[0], 0])) * size
        )
        c11 = (
            o
            + camera.pix2dir(
                torch.LongTensor([camera.intr.resolution[0], camera.intr.resolution[1]])
            )
            * size
        )
    ...
    def plot_pose(cls, pose, size=1, frame=None):
        ...
        x_axis_end_rot = torch.matmul(
            pose.rotation().type(torch.float64), x_axis_end.type(torch.float64)
        )
        y_axis_end_rot = torch.matmul(
            pose.rotation().type(torch.float64), y_axis_end.type(torch.float64)
        )
        z_axis_end_rot = torch.matmul(
            pose.rotation().type(torch.float64), z_axis_end.type(torch.float64)
        )
