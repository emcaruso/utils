...
class PBR_Shader():
    ...
    def render_spherical_loss(cls, cam, obj, scene, rgb_gt, device='cpu'):
        ...
        with torch.no_grad():
            ...
            img = torch.zeros( res ).to(pixels.device)
            diff = rgb.to(img.device) - rgb_gt[pixels[:,1],pixels[:,0],:]
            # img[pixels[:,1],pixels[:,0],:] = torch.abs(diff)
            img[pixels[:,1],pixels[:,0],:] = torch.sqrt(diff**2)
    ...
    @staticmethod
    def render_spherical_pixels(cam, obj, scene, n_pixs=None):
    # def render_spherical_pixels(cam, obj, scene, shading_percentage=1):
        ...
        gbuffers, pixs_all, _ = Renderer.get_buffers_pixels_dirs(cam, obj, n_pixs=n_pixs, channels=['mask','normal','uv'])
        ...
        for i in range(1,3):
            ...
            pixs = cam.sample_rand_pixs_in_mask( mask, n_pixs=None)
            if pixs is None:
                continue
