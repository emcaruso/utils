...
class ArucoGenerator:
    ...
    def generate_aruco_stripe(self, first_id, num_markers, spacing, marker_size=200):
        ...
        canvas_width = (
            num_markers * marker_size + (num_markers - 1) * spacing + 2 * spacing
        )
    ...
    def generate_aruco_stripes(
        self, first_id, num_markers, spacing, num_stripes, marker_size=200
    ):
        ...
        for i in range(num_stripes):
            ...
            stripe, string = self.generate_aruco_stripe(
                f_id, n_mark, spacing, marker_size
            )
    ...
    def generate_aruco_grid(
        self, first_id, spacing, X, Y, border_pixs, marker_size=200
    ):
        num_stripes = Y
        num_markers = X * Y
        stripes, _ = self.generate_aruco_stripes(
            first_id, num_markers, spacing, num_stripes, marker_size=marker_size
        )
        canvas = np.full(
            (
                Y * (marker_size + 2 * spacing + border_pixs) + border_pixs,
                (X * (marker_size + spacing) + spacing) + 2 * border_pixs,
            ),
            0,
            np.uint8,
        )
        ...
        for y in range(Y):
            ...
            stripe = stripes[y]
            canvas[y_ : y_ + (marker_size + 2 * spacing), border_pixs:-border_pixs] = (
                stripe
            )
        return canvas, string

    def generate_aruco_grids(
        self, first_id, spacing, X, Y, border_pixs, num_grids, marker_size=200
    ):
        grids = []
        strings = []
        for i in range(num_grids):
            f_id = first_id + i * X * Y
            grid, string = self.generate_aruco_grid(
                f_id, spacing, X, Y, border_pixs, marker_size
            )
            grids.append(grid)
            strings.append(string)
...
class ArucoDetector:
    ...
    def __init__(
        self,
        aruco_dict=None,
        range_idxs=[0, 9999999],
        detector_params=None,
        color=(255, 0, 255),
    ):
        if aruco_dict is None:
            self.aruco_dict, _ = self.get_aruco_dict(n=4, size=1000)
        else:
            self.aruco_dict = aruco_dict
    ...
    def set_detector(self, detector_params=None):
        ...
        self.detector = cv2.aruco.ArucoDetector(
            dictionary=self.aruco_dict, detectorParams=detector_params
        )
    ...
    def detect_arucos(self, image):
        ...
        marker_corners, marker_ids, marker_rejected = self.detector.detectMarkers(
            image=img
        )
    ...
    def draw_arucos(self, image, rejected=False):
        ...
        if rejected:
            img = cv2.aruco.drawDetectedMarkers(
                img, marker_rejected, borderColor=self.color
            )
        return Image(img)
...
if __name__ == "__main__":
    ...
    # detect arucos
    ad = ArucoDetector(aruco_dictionary)
    img = Image(path="./arucos_test/000.jpg")
    # rotate image by 90 degrees
    # resize img
    # img = img.resize((1200, 1200))
    corn, ids, _ = ad.detect_arucos(img)

    # draw arucos
    img_drawn = ad.draw_arucos(img, rejected=False)
    img_drawn.show()
    # import ipdb
    #
    # ipdb.set_trace()
    ...
    # # cv2.imwrite(f"./arucos/{string}.png", grid)
    #
    # grids, strings = ah.generate_aruco_grids(
    #     100,
    #     spacing,
    #     X=X,
    #     Y=Y,
    #     num_grids=num_grids,
    #     border_pixs=border_pixs,
