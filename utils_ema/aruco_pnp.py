...
class ArucoPnP:
    ...
    def tune_basl_exposure_for_detection(
        self, et=20000, steps=20, mean_range=[50, 200], mean_thresh=10
    ):
        try:
        ...
        for mean_val in mean_vals:
            ...
            ets = fe.tune_exposure(
                exposure_time=et,
                K_et=250,
                key="mean",
                val_target=mean_val,
                val_thresh=mean_thresh,
                start=False,
            )
    ...
    def set_detector(self):
        ...
        self.detector = ArucoDetector(
            aruco_dict, range_idxs=range_idxs, detector_params=None
        )
    ...
    def debug_corners(self, image):
        ...
        if len(marker_corners) == 0:
            print(
                f"{len(marker_corners)} detected markers, less than thresh ({self.cfg.n_aruco_thresh})"
            )
            return False
    ...
    def estimate_pose(self, image, intr, vecs=False):
        ...
        ret, rvec, tvec = cv2.solvePnP(
            corners, marker_corners, camera_matrix, dist_coeffs
        )
    ...
    def draw_pose(self, image, intr):
        ...
        cv2.drawFrameAxes(
            img_new, intr.K_pix.numpy(), intr.D.numpy(), rvec, tvec, length=0.1
        )

        marker_corners, _, _ = self.detector.detect_arucos(image)
        marker_corners = marker_corners.reshape([-1, 2])
        img_new = Image(img_new).draw_circles(
            marker_corners, radius=3, color=(0, 0, 255), thickness=3
        )
